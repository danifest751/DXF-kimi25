...
export class DXFRenderer {
    ...
    ctx = null;
    doc = null;
    rtree = new RTree(config.rendering.hitTesting.rTreeMaxChildren);
    ...
    render() {
        ...
        const viewExtent = Math.max(visibleBounds.max.x - visibleBounds.min.x, visibleBounds.max.y - visibleBounds.min.y) * 2;
        const opts = {
        ...
        for (const idx of visibleIndices) {
            ...
            if (!this.isLayerVisible(fe.effectiveLayer))
                continue;
            if (!fe.entity.visible)
                continue;
     ...
     */
    getEntity(index) {
        if (this.doc === null || index < 0 || index >= this.doc.flatEntities.length)
            return null;
    ...
    // ─── Маркеры врезок ───────────────────────────────────────────────
    setPiercePoints(points) {
        this.piercePoints = points;
        this.requestRedraw();
    }
    get showPiercePoints() {
        return this._showPiercePoints;
    }
    set showPiercePoints(v) {
        this._showPiercePoints = v;
        this.requestRedraw();
    }
    renderPierceMarkers(ctx) {
        if (this.piercePoints.length === 0)
            return;
        const pixelSize = 1 / this.camera.zoom;
