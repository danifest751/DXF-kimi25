...
function bboxFromPoint(p: Point3D): BoundingBox {
  return { min: { x: p.x, y: p.y, z: p.z }, max: { x: p.x, y: p.y, z: p.z } };
}

/** Расширяет bbox точкой */
function expandBBox(bb: BoundingBox, p: Point3D): BoundingBox {
  return {
    min: {
...
function bboxFromPoints3D(points: readonly Point3D[]): BoundingBox | null {
...
function bboxFromPoints2D(points: readonly Point2D[]): BoundingBox | null {
  if (points.length === 0) return null;
...
function bboxLine(e: DXFLineEntity): BoundingBox {
  return mergeBBox(bboxFromPoint(e.start), bboxFromPoint(e.end));
}

function bboxCircle(e: DXFCircleEntity): BoundingBox {
  return {
...
function bboxPolyline(e: DXFPolylineEntity): BoundingBox | null {
...
function bboxPoint(e: DXFPointEntity): BoundingBox {
  return bboxFromPoint(e.location);
}

function bboxSolid(e: DXFSolidEntity): BoundingBox {
  return bboxFromPoints3D(e.points)!;
}

function bboxTrace(e: DXFTraceEntity): BoundingBox {
  return bboxFromPoints3D(e.points)!;
}

function bboxText(e: DXFTextEntity): BoundingBox {
...
function bboxMText(e: DXFMTextEntity): BoundingBox {
  const h = e.height || 1;
...
function bboxHatch(e: DXFHatchEntity): BoundingBox | null {
...
function bboxDimension(e: DXFDimensionEntity): BoundingBox {
  return mergeBBox(bboxFromPoint(e.definitionPoint), bboxFromPoint(e.textMidpoint));
}

function bboxLeader(e: DXFLeaderEntity): BoundingBox | null {
  return bboxFromPoints3D(e.vertices);
}

function bboxMLeader(e: DXFMLeaderEntity): BoundingBox | null {
  return bboxFromPoints3D(e.vertices);
}

function bboxInsert(e: DXFInsertEntity): BoundingBox {
  // Без знания содержимого блока — bbox = точка вставки
  return bboxFromPoint(e.position);
}

function bboxAttdef(e: DXFAttdefEntity): BoundingBox {
  const h = e.height || 1;
  return {
...
function bboxAttrib(e: DXFAttribEntity): BoundingBox {
  const h = e.height || 1;
  return {
    min: { x: e.position.x, y: e.position.y, z: e.position.z },
    max: { x: e.position.x + h * 5, y: e.position.y + h, z: e.position.z },
...
function bbox3DFace(e: DXF3DFaceEntity): BoundingBox {
  return bboxFromPoints3D(e.points)!;
}

function bboxImage(e: DXFImageEntity): BoundingBox {
...
function bboxUnderlay(e: DXFUnderlayEntity): BoundingBox {
  return bboxFromPoint(e.position);
}

function bboxViewport(e: DXFViewportEntity): BoundingBox {
  const hw = e.width / 2;
  const hh = e.height / 2;
  return {
...
function bboxXLine(e: DXFXLineEntity): BoundingBox {
...
function bboxRay(e: DXFRayEntity): BoundingBox {
  const ext = 1e6;
  const endPt: Point3D = {
 ...
 */
export function computeEntityBBox(entity: DXFEntity): BoundingBox | null {
  switch (entity.type) {
 ...
 */
export function computeAllBBoxes(entities: DXFEntity[]): BoundingBox | null {
  let total: BoundingBox | null = null;
