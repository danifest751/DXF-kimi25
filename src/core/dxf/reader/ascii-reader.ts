 ...
 */
export function getValueType(code: GroupCode): 'string' | 'double' | 'int16' | 'int32' | 'int64' | 'boolean' {
 ...
 */
export function parseValue(code: GroupCode, raw: string): DXFValue {
  ...
  while (i < lines.length - 1) {
    ...
    if (isNaN(code)) {
      throw new DXFError(
        ...
        `Невалидный код группы на строке ${i + 1}: "${codeLine}"`,
        'Проверьте, что файл является корректным ASCII DXF.',
      );
    }
    ...
    if (valueLine === undefined) {
      throw new DXFError(
        ErrorCode.PARSE_ERROR,
        `Отсутствует значение для кода ${code} на строке ${i + 1}`,
        'Файл DXF обрезан или повреждён.',
      );
    }
 ...
 */
export function parseSections(groups: DXFGroup[]): DXFSection[] {
  ...
  while (i < groups.length) {
    ...
    if (group.code === 0 && group.value === 'SECTION') {
      ...
      if (i >= groups.length) {
        throw new DXFError(
          ErrorCode.PARSE_ERROR,
          'Неожиданный конец файла после SECTION',
          'Файл DXF повреждён.',
        );
      }
      ...
      if (nameGroup.code !== 2) {
        throw new DXFError(
          ErrorCode.PARSE_ERROR,
          `Ожидался код 2 (имя секции), получен ${nameGroup.code}`,
          'Файл DXF имеет некорректную структуру секций.',
        );
      }
 ...
 */
export function parseAsciiDXF(text: string): DXFSection[] {
