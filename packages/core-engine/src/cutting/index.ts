...
// ─── Извлечение конечных точек сущности ─────────────────────────────

/** Конечные точки сущности (start/end) в мировых координатах */
interface EntityEndpoints {
  readonly flatIndex: number;
  readonly startPt: Point3D;
  readonly endPt: Point3D;
  readonly cutLength: number;
  readonly isSelfClosed: boolean;
  readonly layer: string;
}

/** Извлекает start/end точки и длину режущей сущности */
function getEntityEndpoints(fe: FlattenedEntity, flatIndex: number): EntityEndpoints | null {
  const e = fe.entity;
  ...
  switch (e.type) {
    ...
    case DXFEntityType.LWPOLYLINE: {
      ...
      if (e.closed) len += distPt3(prev, s);
      if (len < EPSILON) return null;
      return { flatIndex, startPt: s, endPt: en, cutLength: len, isSelfClosed: e.closed, layer: fe.effectiveLayer };
    }
    default:
 ...
 */
function buildChains(endpoints: EntityEndpoints[], tolerance: number): ChainInfo[] {
  const n = endpoints.length;
  if (n === 0) return [];

  // Самозамкнутые сущности — отдельные цепочки сразу
  const selfClosed: EntityEndpoints[] = [];
  const chainable: EntityEndpoints[] = [];
  ...
  for (let i = 0; i < chainable.length; i++) parent[i] = i;
  ...
  function gridKey(p: Point3D): string {
    const gx = Math.round(p.x / gridSize);
    const gy = Math.round(p.y / gridSize);
    return `${gx},${gy}`;
  }

  function getNeighborKeys(p: Point3D): string[] {
    const gx = Math.round(p.x / gridSize);
    const gy = Math.round(p.y / gridSize);
    const keys: string[] = [];
  ...
  // Индексируем все конечные точки (каждая сущность даёт 2 точки)
  // pointEntries[k] = { point, entityIdx }
  interface PointEntry { pt: Point3D; entityIdx: number }
  const allPoints: PointEntry[] = [];
  ...
  for (let k = 0; k < allPoints.length; k++) {
    ...
    for (const nk of neighborKeys) {
      ...
      for (const j of list) {
        if (j <= k) continue; // avoid duplicate pairs
        const other = allPoints[j]!;
        if (other.entityIdx === entry.entityIdx) continue; // same entity
...
function isChainClosed(points: Point3D[], tolerance: number): boolean {
...
function transformPt(m: Matrix4x4, p: Point3D): Point3D {
  if (m === IDENTITY_MATRIX) return p;
  return mat4TransformPoint(m, p);
}

function transformPt2to3(m: Matrix4x4, p: Point2D): Point3D {
  return transformPt(m, { x: p.x, y: p.y, z: 0 });
}

function polylineLength3D(pts: readonly Point3D[], m: Matrix4x4): number {
...
function isCuttingEntity(e: DXFEntity): boolean {
  switch (e.type) {
    case DXFEntityType.LINE:
    case DXFEntityType.CIRCLE:
    case DXFEntityType.ARC:
 ...
 */
export function computeCuttingStats(
  doc: NormalizedDocument,
  layerFilter?: ReadonlySet<string>,
  tolerance: number = CHAIN_TOLERANCE,
): CuttingStats {
  // 1. Собираем endpoints всех режущих сущностей
  const allEndpoints: EntityEndpoints[] = [];
  ...
  for (let i = 0; i < doc.flatEntities.length; i++) {
    ...
    if (layerFilter !== undefined && !layerFilter.has(fe.effectiveLayer)) continue;
    if (!isCuttingEntity(e)) continue;
  ...
  const layerMap = new Map<string, { pierces: number; cutLength: number; entityCount: number }>();

  for (const chain of chains) {
    totalCutLength += chain.cutLength;
    if (chain.isClosed) closedContours++; else openPaths++;
  ...
  const byLayer = new Map<string, LayerCutStats>();
 ...
 */
export function formatCutLength(length: number, units: 'mm' | 'm' = 'mm'): string {
  if (units === 'm') {
