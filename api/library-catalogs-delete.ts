...
export default async function handler(req: any, res: any): Promise<void> {
  ...
  try {
    ...
    req.url = '/api/library-catalogs-delete';
    req.originalUrl = '/api/library-catalogs-delete';
    app(req, res);
  } catch (error) {
    const details = error instanceof Error ? error.message : String(error);
    res.status(500).json({ error: 'Library catalogs delete bootstrap failed', details });
  }
}
