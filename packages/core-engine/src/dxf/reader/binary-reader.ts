 ...
 */
export function isBinaryDXF(buffer: ArrayBuffer): boolean {
 ...
 */
function readNullTerminatedString(view: DataView, offset: number): [string, number] {
  const bytes: number[] = [];
  let pos = offset;
 ...
 */
function readBinaryValue(view: DataView, offset: number, code: GroupCode): [DXFValue, number] {
 ...
 */
export function parseBinaryGroups(buffer: ArrayBuffer): DXFGroup[] {
  if (!isBinaryDXF(buffer)) {
    throw new DXFError(
      ErrorCode.INVALID_FILE_FORMAT,
      'Файл не является Binary DXF',
      'Проверьте сигнатуру файла. Ожидается "AutoCAD Binary DXF".',
    );
  }
  ...
  while (offset < view.byteLength - 1) {
    ...
    const code: GroupCode = view.getInt16(offset, true);
    offset += 2;
 ...
 */
export function parseBinaryDXF(buffer: ArrayBuffer): DXFSection[] {
