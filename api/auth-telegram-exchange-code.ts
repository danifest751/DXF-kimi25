...
export default async function handler(req: any, res: any): Promise<void> {
  ...
  try {
    ...
    req.url = '/api/auth/telegram/exchange-code';
    req.originalUrl = '/api/auth/telegram/exchange-code';
    app(req, res);
  } catch (error) {
    const details = error instanceof Error ? error.message : String(error);
    res.status(500).json({ error: 'Auth exchange bootstrap failed', details });
  }
}
