...
function getNum(groups, code, defaultVal = 0) {
...
function getVector3D(groups, xCode, yCode, zCode) {
    return {
...
function parseEntityBase(entityType, groups) {
 ...
 * @returns RGB цвет
 */
export function aciToColor(index) {
...
/** Парсит POLYLINE */
function parsePolyline(groups) {
    const flags = getNum(groups, 70);
    const xCoords = getAllNum(groups, 10);
    const yCoords = getAllNum(groups, 20);
    const zCoords = getAllNum(groups, 30);
    const vertices = xCoords.map((x, i) => ({
        x,
        y: yCoords[i] ?? 0,
        z: zCoords[i] ?? 0,
    }));
...
/** Парсит LWPOLYLINE */
function parseLWPolyline(groups) {
    const xCoords = getAllNum(groups, 10);
    const yCoords = getAllNum(groups, 20);
    const vertices = xCoords.map((x, i) => ({
        x,
...
/** Парсит HATCH */
function parseHatch(groups) {
    // Упрощённый парсинг HATCH — границы как массив точек
    const xCoords = getAllNum(groups, 10);
    const yCoords = getAllNum(groups, 20);
    const zCoords = getAllNum(groups, 30);
    const boundary = xCoords.map((x, i) => ({
        x,
        y: yCoords[i] ?? 0,
        z: zCoords[i] ?? 0,
    }));
...
/** Парсит DIMENSION */
function parseDimension(groups) {
    return {
...
/** Парсит UNDERLAY */
function parseUnderlay(groups) {
    const entityName = getStr(groups, 0);
    let underlayType = 'PDF';
    if (entityName.includes('DWF'))
        underlayType = 'DWF';
    else if (entityName.includes('DGN'))
        underlayType = 'DGN';
    return {
 ...
 */
export function parseEntity(entityName, groups) {
 ...
 */
export function parseEntitiesSection(sectionGroups) {
    const entities = [];
