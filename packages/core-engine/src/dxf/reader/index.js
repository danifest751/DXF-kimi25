 ...
 */
import { DXFFormat, } from '../../types/index.js';
...
import { parseHeaderSection, parseTablesSection, parseBlocksSection, extractMetadata, } from './section-parser.js';
 ...
 */
export function parseDXF(buffer) {
 ...
 * @throws DXFError при ошибке парсинга
 */
export function parseDXFFromString(text) {
 ...
 * @returns Полная модель DXF документа
 */
function buildDocument(sections, format) {
    // Находим секции
    ...
    const entities = entitiesSection
        ...
        : [];
    // Собираем метаданные
    const metadata = extractMetadata(header, format, entities.length, tables.layers.size, blocks.size);
    return {
        metadata,
