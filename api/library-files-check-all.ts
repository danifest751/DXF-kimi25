...
export default async function handler(req: any, res: any): Promise<void> {
  ...
  try {
    ...
    req.url = '/api/library-files-check-all';
    req.originalUrl = '/api/library-files-check-all';
    app(req, res);
  } catch (error) {
    const details = error instanceof Error ? error.message : String(error);
    res.status(500).json({ error: 'Library files check-all bootstrap failed', details });
  }
}
